...
function removeMultipleOp(str) {
  let originalString = str.replace(/\s$/g, "");
  let operators = ["+", "-", "x", "/", "%"];
  let letter = originalString.charAt(originalString.length - 1);
  if (
    operators.some((op) => {
      return op === letter;
    })
...
keys.forEach((key) => {
  key.addEventListener("click", function () {
    ...
    // for dot
      ...
      !Number.isInteger(parseFloat(key.textContent)) &&
      key.textContent === "."
    ) {
      key.classList.add("blink");
      if (num1 === "") {
    ...
    //for operator
      ...
      if (num1 !== "" && operator !== "" && num2 !== "") {
        ...
        recorddiv.textContent = removeMultipleOp(recorddiv.textContent);
